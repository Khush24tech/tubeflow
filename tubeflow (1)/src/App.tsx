...
import { CloudflareSecurityCenter } from './components/CloudflareSecurityCenter';
...
export default function App() {
  ...
  const [downloadQueue, setDownloadQueue] = useState<DownloadJob[]>(() => {
    try {
      const saved = localStorage.getItem('tubeflow_downloads');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('tubeflow_downloads', JSON.stringify(downloadQueue));
    } catch {}
  }, [downloadQueue]);
  ...
  const handleSearch = async (searchQuery: string) => {
    const cleanSearchQuery = sanitizeText(searchQuery);
    if (!cleanSearchQuery.trim()) return;
    setQuery(cleanSearchQuery);
    setIsLoading(true);

    try {
      const res = await fetch(`/api/search?q=${encodeURIComponent(cleanSearchQuery)}`);
      const data = await res.json();
      ...
      if (data.results && Array.isArray(data.results)) {
        ...
        showToast(
          'Results Loaded',
          `Found ${data.results.length} songs and videos matching "${cleanSearchQuery}"`,
          'info'
        );
      } else {
        setResults([]);
  ...
  return (
    <div className="min-h-screen bg-[#0b0f17] text-slate-100 flex flex-col selection:bg-cyan-500 selection:text-white">
      ...
      {/* Footer */}
      <Footer onOpenSecurity={() => setIsSecurityModalOpen(true)} />
      ...
      {/* Cloudflare Security & Device Health Center Modal */}
      <CloudflareSecurityCenter
